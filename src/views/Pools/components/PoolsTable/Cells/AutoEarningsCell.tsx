...
import { Skeleton, Text, useTooltip, HelpIcon, Flex, Box, useMatchBreakpoints } from '@reactswap/uikit'
...
const AutoEarningsCell: React.FC<AutoEarningsCellProps> = ({ pool, account }) => {
  ...
  const labelText = t('Recent REACT profit')
    ...
    <>
      ...
      {hourDiffSinceLastAction ? (
        <>
          ...
          <Text bold>{t('REACT per hour: %amount%', { amount: earnedCakePerHour.toFixed(2) })}</Text>
